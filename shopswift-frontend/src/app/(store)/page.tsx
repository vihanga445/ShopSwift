...
async function getFeaturedProducts(): Promise<Product[]> {
  const res = await fetch(`${API}/api/products/featured`, { next: { revalidate: 60 } });
  if (!res.ok) return [];
  return res.json();
}
  ...
  return (
    <div className="space-y-12 pb-12">
      <section className="bg-slate-50 py-20 text-center">
        <h1 className="mb-4 text-5xl font-extrabold tracking-tight">Shop Smart, Shop Swift</h1>
        <p className="mx-auto mb-8 max-w-lg text-lg text-slate-600">Premium products. Free shipping on orders over $50.</p>
      ...
      <section className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
          {categories.map(cat => (
            <Link key={cat.id} href={`/products?categoryId=${cat.id}`} className="group rounded-lg border p-6 transition hover:bg-slate-50">
